...
type Props = {
  selectedTab: keyof AboutData;
};

export const Tab = ({ selectedTab }: Props) => {
  ...
  const renderTab = (tabCategory: keyof AboutData) => {
    switch (tabCategory) {
      case 'skills':
        return (
          <>
            {aboutData[tabCategory].map((skill) => (
              <div
                key={skill.id}
      ...
      case 'experience':
        return (
          <>
            {aboutData[tabCategory].reverse().map((experience) => (
              <div
                key={experience.id}
                className='flex-1 flex flex-col md:flex-row max-w-max gap-x-2 items-center text-white/60'
              >
                <div className='font-light mb-2 md:mb-0'>
                  <span>
                    ...
                    {experience.companyName !== t('about.tabs.experience.Freelance.name') && 'at'}
      ...
      case 'education':
        return (
          <>
            {aboutData[tabCategory].reverse().map((education) => (
              <div
                key={education.id}
                className='flex-1 flex flex-col md:flex-row max-w-max gap-x-2 items-center text-white/60'
              >
      ...
      case 'awards':
        return (
          <>
            {aboutData[tabCategory].map((award) => (
              <div
                key={award.id}
                className='flex-1 flex flex-col md:flex-row max-w-max gap-x-2 items-center text-white/60'
              >
